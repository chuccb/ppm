...
| Login / channel / join handlers | `Handlers.Auth.cs`, `Handlers.Channel.cs`, `Handlers.Join.Registry.cs`, `Handlers.GL_JOIN*.cs` | authentication、681→143 admission、195→196 channel entry，以及 room-list join flow；每個 GL_JOIN request/ACK family 的 source path 與 entry method 都是 canonical token。 |

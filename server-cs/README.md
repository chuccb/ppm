...
| Room / battle / AI handlers | `Handlers.Room.{Registry,Shared}.cs` + direct `Handlers.{GL,GR,GG}_*.cs`；尚待切分的 `Handlers.{BattleRelay,BattleObjects}.cs`；`Handlers.Ai.Registry.cs` + direct `Handlers.GR_AI_*.cs` / `Handlers.GR_RESET_GAMEROOMSLOT.cs` | 已註冊 Room 與 AI request/ACK family 都以 canonical opcode token 命名（如 `Handlers.GR_MAPCHANGE.cs` / `GR_MAPCHANGE_REQ`、`Handlers.GR_AI_DAMAGE_SHIELD.cs` / `GR_AI_DAMAGE_SHIELD_REQ`）；Registry 與 Shared 僅保留無 callback 的 binding、map compatibility、member/authority support。Battle relay/OCC/drop 保持 source-proven TCP relay 或 fail-closed boundary。 |
| Persistent feature handlers | 暫存 generic domain：`Handlers.Shop.cs`；已改為 direct family files：`Handlers.{Stats,Friend,Clan,Quest,Voice,Warehouse,GameCenter}.Registry.cs` + matching `Handlers.<opcode-family>.cs`，其中僅 `Handlers.{Stats,Voice,Warehouse}.Shared.cs` 是無 receive entry 的 shared wire support。 | feature-domain request parsing 與 response construction；只有 request grammar 與 persistence authority 都已證實時，handler 才可碰 DB mutation。 |

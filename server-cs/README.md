...
| Persistent feature handlers | 暫存 generic domains：`Handlers.{Shop,Stats}.cs`；已改為 direct family files：`Handlers.{Friend,Clan,Quest,Voice,Warehouse,GameCenter}.Registry.cs` + matching `Handlers.<opcode-family>.cs`，其中僅 `Handlers.{Voice,Warehouse}.Shared.cs` 是無 receive entry 的 shared wire support。 | feature-domain request parsing 與 response construction；只有 request grammar 與 persistence authority 都已證實時，handler 才可碰 DB mutation。 |

...
7. `Login` 來自 native `CLobbyLogin` 和 `main:Extracted/ui/login.xml`，不是泛化的
   ...
   並只放 `GL_LOGIN` 的 complete canonical basename/entry。每一個 TCP role 都可收的
   `GT_PING` 則獨立於 `Handlers/GT/` / `GTHandlers`；`GT` 是 catalog prefix，不擴寫其
   未證實的 native semantic。
...
`GC_CLAN_PROTOCOL` sub-op、private UDP opcode、`AI`、`GT` 或 `Login` handler grouping 後，執行

// =============================================================================
// GP_CH*C 戰績家族 (docs/PACKETS.md §3.12) — REQ payload = s32 delta (通常 1),
// ACK = s32 new_value (server 累計後回推; client handler sub_556730 系列)。
// opcode → user_stats 欄位為固定白名單 (防 SQL 注入)。
//
...
{
    private static readonly (Opcode Req, Opcode Ack, string Column)[] Counters =
    [
    ...
    {
        foreach (var (req, ack, column) in Counters)
            add(req, MakeCounter(ack, column));
    }

    private static PacketHandler MakeCounter(Opcode ack, string column) =>
        async (s, p, ctx) =>
        {
            ...
            await s.SendAsync(new Packet(ack).WriteS32((int)value));
        };

    /// <summary>882 推播: s32 累計總遊玩秒數 (client 自行差分)。</summary>
    public static async ValueTask PushPlayTime(Session s, ServerContext ctx)
    {
        if (s.UserId == 0) return;
        long total = ctx.Db.BumpStat(s.UserId, "play_time_s", 0);

...
public sealed class Router
{
    ...
    public async ValueTask<bool> DispatchAsync(Session session, Packet packet, ServerContext context)
    {
        ...
        // claim succeeds. Do not let a pre-handoff socket reach any lobby,
        // economy, social, or room handler merely because it connected to the
        // public channel port.
        if (session.Role is ServerRole.Channel
            && !session.Authenticated
            && packet.Opcode is not Opcode.GT_PING_REQ and not Opcode.PM_UDPSTART_REQ)
        {

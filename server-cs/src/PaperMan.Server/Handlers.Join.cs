...
public static class JoinHandlers
{
    ...
    // 268 GL_JOINPLAY_REQ (u8 room_no u8 flag) → 269 (u8 code)。
    // code 6/7 的成功態需進行中遊戲的完整狀態 (自身 loadout / 全房 +
    // 全玩家 loadout, sub_885D00 尾塊未完全確認) — 本服無遊戲狀態機,
    // 故回 code 0 (client 回房單顯示錯誤), 不硬編未確認欄位。
    private static async ValueTask JoinPlay(Session session, Packet packet, ServerContext context)
    {
        _ = packet.ReadU8();                                // room_no
        _ = packet.ReadU8();                                // 觀戰旗標 (成功態才用)

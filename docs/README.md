...
| [`LAYOUTS.md`](LAYOUTS.md) | opcode 級中心目錄：S2C 收端讀取原語（Part I，306 case 全覆蓋）＋C2S 送端寫入原語（Part II，261 op）＋私人 UDP Appendix A/B＋全 opcode 覆蓋總表（Part III，752 列：官方 676＋無官方名 76，附 registry-only 標注） | 找 op 先翻 Part III 再回對應列；同 opcode 可有多種 builder form，先回到所有 caller；不把線性序列誤讀成完整 payload grammar，重要例外補到 `PACKETS.md`。 |

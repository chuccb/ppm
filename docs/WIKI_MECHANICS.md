...
| 5b-25 | 21 | BotEnemy | `scale` 解析正常・生效 UNRESOLVED（2026-09-18 更正）、死檔 `_intelligent` |
...
| 5b-36 | 32 | 九組同名檔 | `ui/` 根目錄同名 XML **一律是舊副本** |
| 5b-37 | 33 | 戰隊徽章 | 全解：`mark\|frame<<16\|base<<24`、223/105/105；**推翻自己上一版結論** |
| 5b-38 | 34 | 清三個不確定 | C=LOD 階；A 與速度表**證明不在 client**；`move_speed` 從未被讀 |
...
**（2026-09-18 更正：原「scale 死欄位」判決撤回。）** `scale` 出現在每一列，
而 bot 解析器（`sub_8CCB30`）**確實讀取它**——31 個屬性中的第 6 個，
值存入 bot 定義列結構欄位 **+0x1C**，經 `sub_8CE030`／`sub_8CE300`
複製並合入 bot 管理器；`L"scale"` 在 exe 全文實測 **6 次**
（舊文稱 0 次、判為死欄位，與倉內 dump 不符，詳見 RESOURCES §5d-26 更正）。
normal 與 easy 之間**有 7 列的 `scale` 不同**（1.8 vs 2）的觀察仍成立，
有人**刻意調過這個值**，想讓 easy 的怪看起來更大——但**是否生效**
屬 spawn／render 下游消費者問題，**UNRESOLVED**。
教訓本身更加成立：**「資源檔寫了什麼」與「引擎讀了什麼」必須分開查證，
且 grep 斷言必須以倉內 dump 複驗**。
...
| §5b-25 | 資源每列都有 `scale`，引擎**確實讀取**（2026-09-18 更正；**是否生效 UNRESOLVED**）；另有一個零引用的死檔 |
...
### 5b-36. 第三十二輪：九組同名檔 —— 把兩次個案升級成一條通則

本輪起因是查 NewSkill（[ペーパズル/合成](https://wikiwiki.jp/paperman/ペーパズル/合成)、
[リスト](https://wikiwiki.jp/paperman/ペーパズル/リスト)）時，
發現 `NewSkillLevTable.xml` **在 `ui/` 與 `ui/system/` 各有一份**。
詳 [`RESOURCES.md` §5c-3](RESOURCES.md)。

**這不是個案。** 全面對照後：`ui/` 根目錄與 `ui/system/` 共有 **9 組同名 XML**，
而 native 對**全部 9 組只載入 `system/`** ——
exe 中不存在任何從 `ui/` 根目錄載入這些檔名的字串。

其中 **6 組內容相同（無害）、3 組不同（會給出錯誤答案）**：

| 不同的三組 | 讀錯版本的後果 |
|---|---|
| `map_StartIndex.xml` | 拿到**已廢棄**的 `modeStartIndex`(0→5/1→1/3→15)，且**漏掉 GunShooting 與 SOCCER 兩個模式** |
| `voice_customize_contents.xml` | 386 KB vs 2.93 MB —— **少 87% 內容** |
| `FontDefinition.xml` | 缺 `verysmall_size`／`AIWave_size` 兩屬性 |

**方法論上的收穫。** §5e 與 §5d-13 先前**各自**記過前兩個檔「有兩份且值不同」，
但都停在個案，沒人問「這是不是普遍現象」。結果它是 ——
**把兩次孤立觀察升級成一條可套用的規則**，比再記第三個個案有用得多：

> `Extracted/ui/` 根目錄下若有與 `ui/system/` 同名的 XML，
> 一律是舊副本，**只能採用 `system/` 那份**。

這也讓「資源檔誤導分析者」的形態補到**四種**：
死檔（5b-25）、錯置舊副本（5b-26／本輪）、死欄位（5b-24/25/31）、
檔名與內容不符（5b-27）。四者的共同教訓一致：
**檔案系統的呈現不是權威，載入器才是。**

**順帶複查了自己的檢查。** `verify_resource_claims.py` 讀
`ItemAbilityLevTable` 等時都已走 `ui/system/`，無誤。
但本 repo 追蹤的 `Extracted/` 樹**同時存在兩份**，
所以後續新增檢查**必須顯式寫 `system/`**，不可假設檔名唯一 —— 已寫入 §5c-3。

### 5b-37. 第三十三輪：徽章全解 —— 並推翻我自己「無法驗證」的結論

本輪先查 [クラン](https://wikiwiki.jp/paperman/クラン)（2016-02-10）的徽章段，
初稿結論是「120/60/60 **無法由資源導出**」。**那個結論是錯的，原因是我的讀檔方式。**
詳 [`RESOURCES.md` §5d-29](RESOURCES.md)。

**錯在哪。** 我用 `iconv -f CP932` 掃 `ui/ui.xml`，但該檔是 **UTF-8 BOM**，
iconv **靜默回傳空結果**，於是我判定「所有 XML 都沒有格線定義」。
實際上 `ui.xml` 裡就有三個 `customsprites` registry，**逐格列出每一個部件**。
教訓：**空結果要先懷疑自己的讀法**（§5b-32 才因偏移錯誤踩過一次，這次是編碼）。

**全部解出：**

| 項目 | 結果 |
|---|---|
| wire 編碼 | `emblem = mark \| (frame << 16) \| (base << 24)`（`0x37842`）；mark 16-bit、frame/base 各 8-bit，`mark==0xFFFF` 為未選 |
| 部件數 | `EM_MARK` **223**、`EM_FRAME` **105**、`EM_BASE` **105**（`ui/ui.xml` 的 `customsprites`，格子 64×64，每格明列 `key`） |
| 互證 | native `sub_40A630` 把每類分三子頁掃描，邊界 `0/21845/43691`(16-bit 三等分) 與 `0/85/171/255`(8-bit 三等分)；實測 key 起點**逐一對齊**（mark 10..84 / 21845..21919 / 43691..43765） |

**Wiki 的三個數字全部不符 —— 但不是錯，是舊快照。**
...
Wiki 該段自述時點是 **2009-04-28 的素材追加公告**，而本 extraction 是 2016 年版。
frame/base 同為 +45，形態一致 —— 屬**版本差**（同 §5b-19、§5b-33），非矛盾。

**順帶解掉 §5c-2 的一個懸念。** 先前記「emblem 由 UI 兩欄位**相加**得出，
不是三層打包」。真相是兩者不同層：`*(v7+144)+*(v7+128)` 是
`EMBLEMLIST` 的**捲動偏移＋格內索引**（UI 取值），
真正的 wire 打包在 `0x37842`，確實是三欄位位移組合。**原記述誤把 UI 取值當成編碼。**

**仍 UNRESOLVED**：`emblem_base_103/203/303.dds` 三張缺檔
（103 被 13 個 element 引用＝真缺件，203/303 宣告但無人引用＝殘留），
故 base 宣告 105 個中**實可顯示 92 個**；「變更一次 100CASH」仍屬 server 政策。

### 5b-38. 第三十四輪：把三個「查不到」轉成三個確定結論

本輪不讀新 Wiki 頁，而是回頭攻自己承認不確定的三項。
結果**兩項解出、一項升級為可複驗的否定**。

**① `D_Item` 的 `C`(1..4) 是幾何細緻度階梯（解出，Fact / HIGH）。**
先前記「語義未知」。三條獨立證據一致：同一 `(族,等級)` 的四個 C
**除模型路徑與 codeName 外欄位全同**；檔案大小 57 個階差中
**55 個恰為 +1,144 B**；`.NAO` 檔頭兩個計數欄逐階 **+20／+32**，
19/19 群零例外。⇒ 是 LOD／品質階，**與遊戲性無關**。

**② 族序號 `A`(1..7) 對應哪種道具：證明為 client 端不可知（升級）。**
先前寫「無法確定」，語氣像是挖得不夠。本輪給出**否定證據**：
exe 全文中 `D_Item`／`Q_Item`／`P_Item`／`W_Item`／`M_Item`／`DropItem_00`
**六個字串各出現 0 次**，任何落在該 id 區間的數值字面值也**出現 0 次**。
⇒ 客戶端從不指涉任何掉落物，`gameobject.dat` 純粹是
「伺服器下發 id → 模型」的查表資源。**答案只存在於已關閉的伺服器。**
（與 §5b-26「exe 零個 URL 字面值」同型的否定證據。）

**③ Wiki 的武器相對速度表：窮舉確認不在 client（升級）。**
先前只查了 `itemdata.pat` 的 721B 尾段。本輪掃**完整 997B 記錄的每個偏移**
（2,076 筆武器 × s32 與 f32 兩種解讀）：**符合 90..170 或 0.5..2.0 的欄位 0 個**。
加上 `weaponparts.pat` 與 convar 空間（23 個已窮舉）皆無 ⇒
**164/111/105 不落在客戶端**，與 §5b-37 徽章數同類。
...
### 5b-43. 第三十九輪：重讀高影響入口頁，並把「FAQ 敘述」與 client fact 再分開

本輪重讀 [武器移動速度](https://wikiwiki.jp/paperman/武器移動速度)、
[アシストポイント機能](https://wikiwiki.jp/paperman/アシストポイント機能)、
[トーナメント](https://wikiwiki.jp/paperman/トーナメント)、
[試し撃ちシステム](https://wikiwiki.jp/paperman/試し撃ちシステム) 與
[よくある質問や答え](https://wikiwiki.jp/paperman/よくある質問や答え)，並把每頁的
Last-modified 當作該 Wiki 頁的編輯時間，不誤當成 client/server revision。

| Wiki page observation | native/resource cross-check | 結論 |
|---|---|---|
| `武器移動速度`（2023-07-10）說持有武器、角色固有補正、skill、取得 item 都會影響速度；表內另有相對速度列，例如暴走側 164、部分近戰 104–111。 | §5b-29 與 `RESOURCES.md` §5d-11b 已證實 character movespeed、skill speed 軸與 parts 差分的存在，但仍找不到每把武器 base speed 與四路合流計算點。 | 定性對得上；相對數值與公式仍 **UNRESOLVED**，不可寫入 server movement authority。 |
| `アシストポイント機能`（2015-02-22）列出一般 damage/air/heal、爆破、スチール、Pulp、占領、足球；一般頁面寫 55% damage、20% heal 與 50% heal reset，並排除練習/PvE/Single。 | `sub_6750B0`／`994 GG_ASSISTPOINT_NOTIFY` 已證明事由碼 1–3、101–108 與 UI 名稱；client 只顯示事由，沒有自行計分或 account writer。 | 事由字彙是 Fact / HIGH；門檻、點數、reset 與結算仍 **UNRESOLVED**。 |
| `トーナメント`（2013-10-18）說 clan 至少 5 人、每隊最多 5 人、無 item/skill；平日 19/21/23、假日 14/21，並有受付→入場→開始三段與平手判定。 | client 字串／state 已閉合三階段文字與 timing UI；22 個 `*_TNMT_*` opcode 中目前仍只有 764 落地，沒有 server admission、bracket、reward 或 tie-break consumer。 | UI timetable 與文字可作 client fact；參賽資格、賽程、獎勵與勝負政策維持 **UNRESOLVED**。 |
| `試し撃ちシステム`（2012-08-16 實裝）說可重複免費試射，每次 2 分鐘、不可補彈；選中武器外為 MP5K、USP9、CU-BK7、HE GRENADE。 | `itemdata.pat` 的四個唯一 ID 與 local trial/loadout path 已閉合；沒有由該頁或 local path 證明商店 entitlement、計時 authority 或 reward。 | 四個預設裝備／client flow 是 Fact / HIGH；時間執行與商品政策仍不升格。 |
| FAQ（2026-09-06）另以玩家問答形式記錄擊殺連續約 10 秒、擊殺/拾取 PG 量、無墜落傷害、友軍傷害與特殊效果等歷史說法。 | native/resource 目前沒有一條完整 server damage、PG、fall-damage 或 assist settlement proof chain；相關 local UI/weapon data 也不能代替原服 policy。 | 全部保留 **Wiki-only / UNRESOLVED**；FAQ 的更新日期不提高證據等級。 |

這輪沒有把已知的頁面重述誤寫成新 protocol fact。特別是武器變體頁、攻略頁、
FAQ 與 tournament policy 都只能提供候選規則；只有 reader/writer、resource parser、
UI/state consumer 的交叉閉合才提升為 client fact。原服 transaction、score、reward、
anti-cheat、damage authority 與 match admission 仍需 capture 或 server evidence，
不能因 Wiki 已瀏覽而填補。

...
| 5b-37 | 33 | 戰隊徽章 | Wiki 數字**不落在 client**，故不可驗證 |
...
### 5b-37. 第三十三輪：Wiki 給了精確數字，而資源**無法驗證** —— 也要記下來

本輪查 [クラン](https://wikiwiki.jp/paperman/クラン)（2016-02-10）的徽章一節：
「マーク **120 種**／フレーム **60 種**／ベース **60 種**、變更一次 **100CASH**」。
結果是**部分可證、關鍵數字不可證**。詳 [`RESOURCES.md` §5d-29](RESOURCES.md)。

| Wiki 敘述 | 比對結果 |
|---|---|
| 徽章由**三層**（マーク／フレーム／ベース）組成 | **不可由 native 證實**。exe 的 `emblem_1..4` 節點是「同時顯示 N 個圖示時的座標」版面預設（且 `present_1..4` 形狀完全相同），**與三層無關**。初稿誤把它當成三層，經 mutation 測試抓出。 |
| — | **另得**：wire 上只有**一個 `s32`**（585 REQ 第四欄），且由 UI `EMBLEMLIST` 以**兩欄位相加**算出 —— **不是**三層 id 的簡單打包。 |
| マーク **120**／フレーム **60**／ベース **60** | **無法導出，不採用**。隨附 21 張都是**圖集**，而**所有 XML 都沒有格線定義**，**exe 也從未出現這些檔名**。以 64×64 格推算容量足夠（3×64=192），但**容量 ≠ 數量**。 |
| 變更一次 100CASH | **client 無證據**，屬計費政策，UNRESOLVED。 |
| 等級 3 階（マスター／サブマスター／一般）、Lv1–10 人數上限 20→100、クラン経験値表 | **本輪未查證**；等級表屬 server policy，列為後續線索。 |

**為什麼要把「查不到」也寫成一節。**
前幾輪多半能交叉驗證，容易讓人以為 Wiki 的數字總能對上。
本輪正好相反 —— 而原因不是 Wiki 不精確，是**那個事實根本不落在客戶端資產裡**：
圖集沒有格線定義、檔名不出現在 exe，**客戶端沒有「有幾種」這個資訊**。

與 §5b-23 恰成對照：耐久那輪資源反過來修正了 Wiki 的 19%→20%，
因為門檻確實編碼在 `durable_ability.xml` 裡。
**能否交叉驗證取決於該事實是否落在 client 端，而非 Wiki 寫得精不精確** ——
這條判準比再記一個個案有用。

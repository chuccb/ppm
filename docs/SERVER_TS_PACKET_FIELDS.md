...
| 434 `GL_FRIEND_LIST_ACK` | raw2/self/count；consumer keeps at most 100 nickname/row-field rows, 21-byte nickname slot, wire raw4 row field with local low-byte retention。 | 無。 | self is display context, not proven owner id。 | header 0/self/count 0。 | header/row-field values、friend policy。 |
...
| 425 → 426 | `sub_55A580` writes a 4-byte value from `sub_44E480`; `sub_6DC6C0`/`sub_6DC760` adjust the UI value by −/+10 (the previous path floors at 1) | `sub_55A630` reads raw2/self/count and records; `sub_5378C0` stores at most 10 | 425 is natively a message-list navigation scalar; 426 UI consumes records and the two raw fields, but no server mailbox/page policy is recovered | no Extracted mailbox schema; fixed local capacities are client facts |
| 433 → 434 | empty friend-list request through dispatcher | `sub_55AFC0` reads raw2/self/count; `sub_537F60` stores up to 100 rows and assigns wire raw4 row field into a one-byte local slot | self display name is stored separately; the row field is 4 bytes on wire but recovered local storage visibly keeps only low 8 bits | no friend resource; header/row-field domain unresolved |
...
| 434 `GL_FRIEND_LIST_ACK` | `raw2 rawHeader`, `str self`, `u8 count`; each record: `str nickname`, `raw4 field_a3` (native local table keeps low byte) | **Native fact:** `sub_55AFC0` reads the two-byte header, self string, and unsigned count; `sub_537F60` stores at most 100 nickname/row-field records. **Resource fact:** the native table count is at `this+244236`; insertion is allowed only while count `< 0x64` (100 entries), each nickname uses a 21-byte NUL-string stride at `this+244237+21*index`, the wire `int` record field is assigned into the one-byte slot at `this+61585+index` (only its low byte is visibly retained), and the surrounding delete/shift paths retain two one-byte side fields per slot. `sub_537F60` itself copies until NUL without visibly clamping to the 21-byte stride. `Extracted/` has no friend-table or row-field definition that can name these fields. **Inference:** the self string is the local display-name context because the native consumer stores it separately from friend rows; it is not proven to be an owner id. **TS projection:** header 0, authenticated account nickname, count 0. **UNRESOLVED:** header domain, friend nickname/row-field domain, row-field values, side-field meanings, and friend policy. | `sub_55AFC0`、`sub_537F60`（100-entry cap、21-byte key stride、wire raw4 row field→local low byte、side-field shift paths）；HIGH for order/width/capacity, UNRESOLVED business fields |

...
| 105 `GL_USERLIST_REQ` | `sub_56A0F0` accepts unsigned `a2`; only `a2==1` passes the one-second `timeGetTime` gate and writes one byte, while `a2==0` only changes the caller's local transition path. | 無。 | refresh trigger byte, not a proven status/filter. | consumes exactly one byte, returns 106 zero gate. | trigger values other than native `1`, rate policy。 |

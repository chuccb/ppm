...
| 200 `GL_MYITEM_ACK` | `sub_570AB0` reads one success byte; if nonzero, `sub_524B70(...,1)` reads start plus up to 100 records in the 5120-slot array, then all paths call `sub_41BF20(byte_BF0724)` (local state 8→9) and set `byte_EE8C05=1`. Record grammar is `s32 slot, s32 item_id, f32, f32, s32, u8, u16`; negative slot stops, negative/catalog-missing item id enters native error handling, and `sub_534450` updates the client item catalog's durability words. | `Extracted/ui/cfg/itemdata.pat` is native catalog membership source。 | period is duration-like only from consumer context; 200 completion also advances a local inventory state flag, not a server success policy. | success 1/start 0, optional record API, `-1` terminator。 | f1/f2/extra domains、ownership/service/time policy、native error-code projection。 |

...
| 108 `GL_GAMEROOMINFO_ACK` | `u8 mode,u8 count` then count records；`mode==3` diverts to `sub_580A80` before ordinary count records；zero count stops ordinary branch。 | `maplist.pat`/`Map.dat` are map catalogs, not room-state authority。 | mode 0 is ordinary room-list; mode 3 is a separate native branch。 | mode 0/count 0 conservative projection。 | mode-3 body, room record fields/policy。 |
...
| 681 `GL_LOGIN_ACK` | success prefix, raw2 server/group fields, positive-capacity one-channel body, type-3 extra, billing tail；failure only result。 | 無 resource authority for billing/flag/group。 | channel field is USERS current numerator, server port is selector endpoint；capacity/current-user raw2 are consumed as signed short by the native path。 | three fixed groups, s16/raw2 width checks, zero gates only for omitted groups, positive gate requires a complete channel body、ext gate 0。 | flag/group/billing/user_no domains、server policy。 |
...
| 107 → 108 | lobby refresh caller sends empty 107 | `sub_568CE0` reads `mode`; mode 3 diverts to `sub_580A80`, otherwise reads count and ordinary rows | mode 0 + count 0 terminates before room-record consumer；room state is not fabricated | `maplist.pat`/`Map.dat` do not prove server room policy；mode 3 unresolved |
...
| 425 → 426 | `sub_55A580` writes signed value from `sub_44E480`; UI also adjusts ±10 | `sub_55A630` reads raw2/self/count and records; `sub_5378C0` stores at most 10 | message-list UI consumes records and signed id/date, but no page/mailbox server consumer is recovered | no Extracted mailbox schema; fixed local capacities are client facts |
| 433 → 434 | empty friend-list request through dispatcher | `sub_55AFC0` reads raw2/self/count; `sub_537F60` stores up to 100 rows | self display name is stored separately; friend status is an `int`, but no policy table is recovered | no friend resource; header/status domain unresolved |
...
| 426 `GL_MSG_RECVLIST_ACK` | `raw2 rawHeader`, `str self`, `u8 count`; each record: `str from`, `u8 raw`, `str title`, `s32 msg_id`, `str body`, `str raw`, `s16 date` | **Native fact:** `sub_55A630` reads the message id into `int v37` and the date into `__int16 v35`; its `sub_5378C0` consumer retains at most 10 records. **Resource fact:** the native table count is at `this+241704`; per-entry storage uses fixed strides of 20 bytes for the first NUL string, 2 bytes for the raw byte, 21 bytes for the second NUL string, one byte for its value, 201 bytes for the third NUL string, 2 bytes for the final NUL string, and 2 bytes for the final signed-short value. The copy loops themselves stop at NUL and do not visibly clamp to those strides. No Extracted message/catalog resource gives the two raw fields or a mailbox schema. **Inference:** none beyond the record being a client message-list projection. **TS projection:** count 0 with `rawHeader=0` and the authenticated self display name; no fabricated records. **UNRESOLVED:** rawHeader, sender/title/body/raw semantics, date epoch, mailbox paging/policy. | `sub_55A630`、`sub_5378C0`（10-record cap、fixed storage strides、`int a5`、`__int16 a8`）；HIGH for order/width/capacity, UNRESOLVED business fields |
...
| 434 `GL_FRIEND_LIST_ACK` | `raw2 rawHeader`, `str self`, `u8 count`; each record: `str nickname`, `s32 status` | **Native fact:** `sub_55AFC0` reads the two-byte header, self string, and unsigned count; `sub_537F60` stores at most 100 nick/status records. **Resource fact:** the native table count is at `this+244236`; insertion is allowed only while count `< 0x64` (100 entries), each nickname uses a 21-byte NUL-string stride at `this+244237+21*index`, the `int` status is stored at `this+61585+index`, and the surrounding delete/shift paths retain two one-byte side fields per slot. `sub_537F60` itself copies until NUL without visibly clamping to the 21-byte stride. `Extracted/` has no friend-table or status definition that can name these fields. **Inference:** the self string is the local display-name context because the native consumer stores it separately from friend rows; it is not proven to be an owner id. **TS projection:** header 0, authenticated account nickname, count 0. **UNRESOLVED:** header domain, friend nickname/status domain, status values, side-field meanings, and friend policy. | `sub_55AFC0`、`sub_537F60`（100-entry cap、21-byte key stride、`int` status、side-field shift paths）；HIGH for order/width/capacity, UNRESOLVED business fields |

...
| 107 `GL_GAMEROOMINFO_REQ` | 空 request；對應 108 reader。 | 無直接 room payload 對應。 | lobby room refresh。 | 嚴格空讀，回 `mode=0,count=0`。 | room model 與 policy。 |
...
| 195 `GC_ENTERCHANNEL_REQ` | 三個 `u8`；第三 byte 來自 local option block。 | 無。 | 前兩 byte 對應 681 的 group/channel 選擇。 | 驗認證與 group/channel；rawFlag 不作放行條件。 | rawFlag domain、type-3 server policy。 |
...
| 425 `GL_MSG_RECVLIST_REQ` | `s32`；native UI 初值 1，另有 ±10 writes。 | 無 mailbox resource/schema。 | list navigation 是 plausible，但未由 wire alone 命名。 | 讀取並忽略，回 zero-count 426。 | page/mailbox semantics、range policy。 |
| 433 `GL_FRIEND_LIST_REQ` | 空 request，對應 434。 | 無 friend table/status resource。 | current-account friend-list request。 | zero-count 434 + self display name。 | friend storage/status/policy。 |
...
| 108 `GL_GAMEROOMINFO_ACK` | `u8 mode,u8 count` then count records；zero count stops。 | map resources do not prove room policy。 | ordinary room-list branch for mode 0。 | mode 0/count 0。 | room record fields/policy。 |
...
| 196 `GC_ENTERCHANNEL_ACK` | failure is 3 fields; success appends endpoint/type; type 3 calls gated continuation。 | 無 direct type-3 catalog。 | endpoint is subsequent UDP control handoff。 | success/failure framing and optional type-3 raw tail are preserved。 | endpoint policy、opaque flags、type-3 record semantics。 |
| 681 `GL_LOGIN_ACK` | success prefix, raw2 server/group fields, positive-capacity one-channel body, type-3 extra, billing tail；failure only result。 | 無 resource authority for billing/flag/group。 | channel field is USERS current numerator, server port is selector endpoint。 | three fixed groups, width checks, safe zero projection、ext gate 0。 | flag/group/billing/user_no domains、server policy。 |
...
| 255 `GL_INVENIN_ACK` | common 4-field prefix；mode 0 remote branch；mode 1 selected `<5` then 5×32-byte raw profiles。 | NewSkill XML and shipped item catalog identify seven puzzle controls/ID space。 | uid/profile scope is account-level, not character appearance。 | mode 1 only; echo context; five 7-ID + expiry records。 | remote fields, header/context, catalog completeness, expiry/ownership policy。 |
| 426 `GL_MSG_RECVLIST_ACK` | count is `u8`; client stores at most 10 records; id `int`, date `__int16`, fixed local string slots。 | 無 message schema。 | message-list projection, not proven page model。 | raw2/self/count 0。 | header, record raw fields, date epoch、mailbox policy。 |
...
| 434 `GL_FRIEND_LIST_ACK` | raw2/self/count；consumer keeps at most 100 nick/status rows, 21-byte nickname slot, int status。 | 無。 | self is display context, not proven owner id。 | header 0/self/count 0。 | header/status values、friend policy。 |
...
| 195 `GC_ENTERCHANNEL_REQ` | `u8 group`, `u8 channel`, `u8 rawFlag` | `group` 是 681 廣告的 group 序號，`channel` 是組內 channel index。第三 byte 是由 local option block 產生的 raw flag；`sub_735DE0` 只證明 native 讀取一個設定值，不能直接命名成 replay/module/status。TS 讀取但不以它放行。TS 僅接受已認證且 group/channel 與 config 相符；type-3 continuation 可省略或使用 native header0-only boundary，不把缺少 continuation 當成 server policy。 | `sub_56FF40`、`sub_4179D0`、`sub_7338D0/sub_735DE0`；HIGH for shape, UNRESOLVED for rawFlag domain |
...
| 426 `GL_MSG_RECVLIST_ACK` | `raw2 rawHeader`, `str self`, `u8 count`; each record: `str from`, `u8 raw`, `str title`, `s32 msg_id`, `str body`, `str raw`, `s16 date` | **Native fact:** `sub_55A630` reads the message id into `int v37` and the date into `__int16 v35`; its `sub_5378C0` consumer retains at most 10 records, with native local capacities of 20 bytes for sender, 21 for title, 201 for body, and 2 bytes for the final raw string storage. **Resource fact:** no Extracted message/catalog resource gives the two raw fields or a mailbox schema. **Inference:** none beyond the record being a client message-list projection. **TS projection:** count 0 with `rawHeader=0` and the authenticated self display name; no fabricated records. **UNRESOLVED:** rawHeader, sender/title/body/raw semantics, date epoch, mailbox paging/policy. | `sub_55A630`、`sub_5378C0`（10-record cap、fixed local arrays、`int a5`、`__int16 a8`）；HIGH for order/width/capacity, UNRESOLVED business fields |
...
| 434 `GL_FRIEND_LIST_ACK` | `raw2 rawHeader`, `str self`, `u8 count`; each record: `str nickname`, `s32 status` | **Native fact:** `sub_55AFC0` reads the two-byte header, self string, and unsigned count; `sub_537F60` stores at most 100 nick/status records, with a 21-byte nickname slot and an `int` status. **Resource fact:** `Extracted/` has no friend-table or status definition that can name these fields. **Inference:** the self string is the local display-name context because the native consumer stores it separately from friend rows; it is not proven to be an owner id. **TS projection:** header 0, authenticated account nickname, count 0. **UNRESOLVED:** header domain, friend nickname/status domain, status values, and friend policy. | `sub_55AFC0`、`sub_537F60`（100-record cap、21-byte nickname storage、`int` status）；HIGH for order/width/capacity, UNRESOLVED business fields |

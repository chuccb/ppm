...
### 5d-29. 戰隊徽章：三層結構已證，但 Wiki 的「120/60/60」**無法由資源導出**（三十三輪）

[クラン](https://wikiwiki.jp/paperman/クラン)（2016-02-10）在徽章一節寫
「マーク 120 種／フレーム 60 種／ベース 60 種，變更一次 100CASH」。
本輪查證結果是**部分可證、關鍵數字不可證**，兩者都記下來。

**已證（Fact / HIGH）：**

1. **版面解析器支援 1..4 個圖示位，不是三層。**
   native 有兩處 `emblem_N` 解析器：`0x546112` 與 `0x546165` 各處理
   `emblem_1/2/3`（`pos_1` / `pos_1..2` / `pos_1..4`），
   而 `0x546213` 另多一個 **`emblem_4`**（`pos_1..4`）。
   同一函式緊接著以完全相同的形狀解析 `present_1..4`。
   ⇒ 這組節點是**「同時顯示 N 個圖示時各自的座標」**（N=1..4 的版面預設），
   **不是** Wiki 所說的「マーク／フレーム／ベース 三層」。
   兩者不可對應 —— 本輪初稿曾誤寫為三層，經 mutation 測試發現並更正。
2. **wire 上只有一個 `s32`。** 585 `GC_CLAN_CREATE_REQ` 的第四欄是
   `s32 emblem`（§3 已於廿四輪把 u8 更正為 s32）。
3. **該值由 UI 的 `EMBLEMLIST` 控制項算出，且是「兩個欄位相加」**
   （`0x144555`：`sub_5505F0(..., *(v7 + 144) + *(v7 + 128))`）——
   **不是**單純把三層 id 打包成一個整數。
4. **隨附 21 張圖集**：`mark` 512×512 ×6、`frame` 256×256 ×6、`base` 256×256 ×6
   （另 `frame` 有第三組變體共 9 張），皆為未壓縮 32-bit（`512*512*4+128`
   與檔案大小完全相符）。

**無法證（維持 UNRESOLVED，不採用 Wiki 數字）：**

- **120／60／60 這三個數字導不出來。** 21 張都是**圖集**，
  而**所有隨附 XML 都沒有對應的格線定義**（`clan_emblem.xml`／
  `change_emblem.xml` 皆為純版面），**exe 全文也從未出現這些檔名**。
  以 64×64 格推算單張 512×512 可容 64 格、三張 192 格 —— **容量足夠但不等於數量**。
  在找到格線定義前不得把 120/60/60 寫進實作。
- `_1xx`／`_2xx`／`_3xx` 後綴分組的語義未明（疑為顯示尺寸階，但無證據）。
- 「變更一次 100CASH」屬 server 計費政策，**client 無證據**。

**方法論註記。** 這是少數「Wiki 給了精確數字、資源卻無法驗證」的案例。
與 §5b-23（耐久 19%→20%，資源可反過來修正 Wiki）恰成對比：
**能否交叉驗證取決於該事實是否落在客戶端資產裡**，
而不是 Wiki 寫得精不精確。
...
- **但 Wiki 那張「相對速度」數表無法由現有資源重建。**
  `partsability.pat` 只給**改裝件差分**；**每把武器的基礎速度**
  在 `itemdata.pat` 的 721B 尾段中**找不到對應欄位**（§2c 已切段完畢），
  在 `weaponparts.pat`（純相容性邊）中也沒有。
  因此 Wiki 的 164/111/105 這類數字**來源不明，維持 UNRESOLVED**，
  **不得寫死**（§1 的禁令依然有效）。
- 合成順序（相加？相乘？先後？）同樣**未證實**：
  本輪只確立「改裝件這一路的輸入來自槍托」，
  沒有找到把角色、skill、武器、道具四者合流的計算點。

...
**音源根與「語音商品」是兩個不同的軸（重要區分）——但對位已解開一層：**
磁碟實際三個根 `sounds00`(14 角色)／`sounds01`(9)／`sounds80`(15)，
而 §5c-2b 的商品目錄是 **11 個語音套組**（`Voice F`/`A`/`S`/…）。
本輪（新輪）經 `voice_customize_path.xml`（sounds%02d 每槽一根）＋
`voice_customize_contents.xml` 台詞 × Wiki 按鍵表三交叉，**VoiceNN 槽 ↔
套名映射已定 34 槽**（F=01、A=02、S=03、Shuffle=04、Cafe=05、Action=06、
school=07、A2=08、A3=09、F2=10、F3=11、S2=12、S3=13…其餘見 §11.6 映射表）
——商品套組 ↔ 槽位這一軸**已閉合**（初期 11 套恰好佔槽 01–11，Shuffle
系則在 04/46/78 三槽各換聲優）。仍保留的 UNRESOLVED 只剩**資產缺席**：
92 槽中僅 3 根在磁碟有實體聲音（§11.6 catalog-ahead 分析）。
...
### 5e-2. thumb／pav 例外集合の版本考古（本輪定案；§11.3 例外條目指向本節）

§11.3 的兩組資產例外（38 個例外 thumb id＋7 個例外 pav 檔）在此完成
時間層考古與四族分類。結果：**證偽「單一凍結快照外洩」說**，支持「營運期間
常態性資產先行、最終未上架或刪行」的常態工程殘留說（Probable）。

#### 方法定案（Fact，可複驗）

- **內嵌日期戳**：997B 記錄的偏移 **+981..+988＝YYYYMMDD 八位元組**。
  全 21,164 筆中 **21,109 筆（99.74%）帶合法日期**；55 筆無日期
  （分布 帶 10xxxxxx:10、12xxxxxx:8、15xxxxxx:37）。
- **考古三元**：例外 id 自身無記錄 → 改以其同帶鄰居窗（low5 同帶 ±40..55）
  的日期與名稱，加上資產本身的存在/目視，判定資產的所屬年代與物種。
- ⚠ **git 日期不可用**：`origin/main` 全樹為 2026-09-18 整批快照導入，
  `git log` 對任何檔案均回該日——版本考古一律走本節內嵌日期路線。

#### 例外四族分類（38 thumb 例外＋4 組 pav 例外）

**① run 內缺口（catalog 刪行、資產留存）——3 組，與 pav 例外 7 檔完全互證：**

| 例外 id | 殘存資產 | 證據 run（同帶連號）| 鄰名（Fact）| 鄰日期 |
|---|---|---|---|---|
| 10200310 | thumb＋`02_00310_{01,02}` | 309/311 在冊、310 缺口 | ウィルキンソンTシャツ（兩鄰同名）| 20110629 |
| 10500494 | thumb＋`05_00494_{01,02}` | 493/495 在冊、494 缺口 | コラボユニフォーム（兩鄰同名）| 20110629 |
| 10600228 | thumb＋`06_00228_01` | 227＝墨マーク、229＝アッシュサングラス | thumb 目視＝**骷髏海盜眼罩** | 帶 106＝eye（§11.3a）|
| 10000408 | **僅 pav** `00_00408_{02,04}`（無 thumb）| 407/409 在冊、408 缺口 | 三ツ矢サンバイザー（兩鄰同名）| 20110629 |

**四組刪行中三組（310／494／408）共享同一鄰日期 20110629**——同一批
catalog 編修在 2011-06-29 刪掉三行（Strongly Supported 單批操作）。
7 個 pav 例外檔至此**全數對號**（6＋2＝7 檔、4 個 id）。

註：舊記「106＝faceAcc」屬誤植——f1＝06 與 §11.3a 十 accessor 的 eye 帶
一致，pav 檔名與 thumb 帶自證。

**② 整塊未入冊——12 連號 15400407..418（avatar／戦闘服帶 1540）**

- 前鄰 15400370＝`ハヤテ戦闘(Voice Archer)`（20120523）、後鄰 15400419＝
  `ハヤテ戦闘(Voice Madoka)`（20130116）、再後 15400447＝
  `ドッドン戦闘(Voice Sayaka)`（20130116）——**前後都是在冊的「戦闘
  (Voice ◯◯)」異聲優版戰鬥服系列**。括號名詞（Archer／Madoka／Sayaka）
  指向動畫聯名（Fate 系／魔法少女まどか☆マギカ系），惟聯名活動本身未另行
  查證，僅據冠名。
- 全 12 張 thumb 已解包目視兩張：15400407＝棕髮男性戰鬥服角色卡、
  15400408＝金髮女性戰鬥服角色卡，卡面帶紅星＋喇叭（Voice）標誌——
  正是「戦闘(Voice ◯◯)」系列的卡式縮圖風格。
- 結論（Probable→Strongly Supported）：407..418 是一**整套 12 件聲優
  聯名戦闘服（或其角色卡）**，縮圖已全部繪製完成，但該批 12 id **整批
  未入 catalog**（不像 309/310 型單點刪行）。夾層位置（2012-05 與
  2013-01 兩聯名之間）暗示它們屬於某個**取消或未上架的聲優聯名批次**。

**③ 帶 15305 散件——9 件（同帶雜區）**

- `15300553/1104/1107/2002/2003`（前段）、`15305054/5055`、
  `15305608/5708`。鄰窗證據：5054/5055 附近為 **ニコクロスヘアスキン
  （20140514）**，5608/5708 附近為 **EXP+50%UP（20090422）**——年代
  跨度即帶 15305 本身的混居性（語音商品亦在本帶，§5c-2b）。

**④ 武器缺憾——8 件（帶 121／122）**

| 例外 id | 鄰窗年代 | 備註 |
|---|---|---|
| 12100145、12100276 | 2009–2011 | — |
| 12100597、12100605 | 2011–2014 | — |
| 12102087 | 2013（後鄰 12102088＝プラズマガン (PVE)，20131127）| — |
| **12100800/801/802** | 2015–2016（前鄰 12100769＝アルティメイトロケットランチャー 20151014）| thumb 目視 800＝**「SSW Revolver」手槍**；「SSW」標記亦見於**在冊** 12202971＝Kriss Vector(SSW)（20160323）→ SSW＝在冊標記系列，**涵義 UNRESOLVED** |

`12100801/802` 與 800 三連號同批；803..806 兩側皆無（thumb、itemdata 皆
缺）→ 800–802 是該帶唯一的資產存留洞而非 run 截斷。

#### 考古結論（Strongly Supported）

- 四族例外的鄰窗日期橫跨 **2009-04-22 → 2016-03-23 整個營運期**，
  無單一時間層——例外集合不是一次洩漏或凍結的截面，而是長年累積。
- 對私服選資產的啟示：例外 thumb/pav 可用（圖在、檔在），但**沒有任何
  catalog 行**——要啟用須自建 itemdata 記錄與價格欄（參照同帶鄰居行
  結構）；② 之 12 連號若復原其「戦闘(Voice ◯◯)」系列語意，還需對應
  voice slot（§11.6 映射表）的聲包素材。
...
- **thumb（Confirmed）**：`item/thumb/N.tga` 的純數字名＝**完整 item id**。
  ...
  `12202962..65`、`10600228` 等）＝資產存在但 catalog 無此 id——
  **時間層考古已定案於 §5e-2**（四族分類：run 內刪行 3 組／整塊未入冊 12 連號／
  帶 15305 散件 9 件／武器缺憾 8 件，鄰窗年代全跨 2009–2016 營運期）。
...
- **pav 7 例外**：`00_00408_{02,04}`、`02_00310_{01,02}`、`05_00494_{01,02}`、
  `06_00228_01`——**全數＝§5e-2 族①的 run 內刪行留資產**（鄰名：三ツ矢
  サンバイザー／ウィルキンソンTシャツ／コラボユニフォーム／眼帯サングラス族；
  前三組同日 20110629 刪行）。`(6,00228)` 並與 thumb `10600228.tga` 互證
  （骷髏海盜眼罩，目視）；`00_00408` 為唯一「pav 存、thumb 不存」的刪行個案。
...
- **文本鏡像＝`ui/system/voice_customize_contents.xml`**（UTF-16LE BOM；
  root `vcustomizelistTable`）：**15 個角色塊**（hayate/tina/milly/alulu/
  van/hood + Cyrus/Doddon/Guy/Tericia 用羅馬字顯名；spy_11/robotgirl_12/
  tsunderegirl/magicgirl/devilgirl 用代號標籤——新舊命名習慣混存；
  舊記「11 塊」為漏數，2026-09-18 更正）× **同空間 Voice00..92** ×
...
方法：`voice_customize_contents.xml` hayate 塊每槽 27 條台詞（command/
tactics/information_1..9），Wiki「ラジオチャット一覧/ハヤテ」每套 Voice
為 3×9 按鍵表（Z 指揮／X 戦術／V 報告）——同構對位，2–3 句獨特台詞
即可定槽。已遍覽全頁台辭段落（chunks 0–5；其餘 10 chunks 為站內導航，
無新套名；Last-modified 2016-07-29）。ハヤテ頁名冊合計 53 套。

**Confirmed（多句原文命中或方言指紋）15 槽：**

| 槽 | 套 | 判別句／依據 |
|---:|---|---|
| 00 | 標準（保志総一朗）| Z1 攻撃を受けてる！援軍を頼む！＝Wiki 標準（標點微差）|
| 01 | Voice F（某国の王子）| 援護はどうした／まぐれか？／褒めてやろう 三句全中 |
| 03 | Voice S（陰陽師）| 怨敵がおるぞ／討ち取ったり！ 等三句 |
| 09 | Voice A3 | デモリッションを装備しました 等三句 |
| 10 | Voice F2 | Z1 よろしく／X1 勉強させてもらう 原句 |
| 11 | Voice F3 | 準備はできているだろうな？／ソナ…レーダー 二句 |
| 12 | Voice S2 | よろしくのぉ／おぬし奇遇じゃのう 等三句 |
| 13 | Voice S3 | 準備は万全かの？／邪気をレーダーに送ろうぞ 等三句 |
| 14 | Voice Angel | え・・援護・・援護を 等三句 |
| 15 | Voice Devil | え・・援軍を貰いたいな 等三句 |
| 18 | Voice Yankee | ひっ、援護・・援護をください 等三句 |
| 26 | イカ娘聯名 | killmsg=1 及前文已證 |
| 55 | Voice 妖怪 | 手伝いがほしいねぇ／皆でかかるぞぉ／ほいさ 三句序位一致 |
| 84 | 関西弁（2015/07、大空直美）| わっ！攻撃や！援護頼むで——**関西弁方言指紋**＋CASH 頁公告＋在冊品 `15307095 ボイス袋 関西/博多袋`（§5c-2b）|
| 85 | 博多弁（2015/08、阿澄佳奈）| 援護頼んだけーん／みんなで突撃ばーい！——**博多弁指紋**（同袋）|

**Confirmed 續（本輪新對位，19 槽——原文全句命中 16、偽名／點標記變體 3）：**

| 槽 | 套 | 判別句 |
|---:|---|---|
| 19 | Voice Horror（気弱）| こんな体ですが、援護が欲しいです（全句）|
| 20 | Voice Pirate（熱血海賊）| 俺を助けろ！（全句）|
| 21 | Voice Band（厳格バンドマン）| ヘルプ、頼む（全句）|
| 22 | Voice Hunter（熱血ハンター）| 神が援護を求めている（全句）|
| 23 | Voice Magician（厳格）| アシスタント、頼んだぞ（全句）|
| 24 | Voice Dracula（気弱）| た・・・助けて～（全句）|
| 25 | Voice Army（英語）| ニードバックアップ【Need backup】（全句；11/15 塊共稿）|
| 27 | Voice Wrestler（横暴）| ヘルプに来いや！（全句）|
| 28 | Voice Kung-Fu（熱血）| 敵襲だ、援護を！（全句）|
| 29 | Voice Pilot（気弱 15歳）| っ、敵襲だ！援軍を！（全句）|
| 30 | Voice Adventurer（気弱 14歳）| わわわわ！誰か助けて～！（句尾長音符差）|
| 31 | Voice Teacher（魅惑 27歳）| 困ったな、こっちに来てくれないかい？（全句）|
| 42 | Voice Mononofu（血気 武将）| 至急後詰め（ごづめ）を！（全句）|
| 43 | Voice Academy（生意気 候補生）| 前方から威嚇射撃！応戦を！（全句）|
| 45 | Voice 貴族（物腰やわ 22歳）| ご援護いただけませんか？（全句）|
| 56 | Voice Refine | くっ！援軍はまだか！（全句）|
| 57 | Voice Summer School | 支援がほしい・・・↔欲しい･･･（偽名・中點差）|
| 59 | Voice 俺様（15歳）| 援護したいなら受けてやってもいいぜ（全句）|
| 72 | Voice 地球防衛軍（22歳）| 被弾だ・・・助けに来い↔･･･助けに来い（中點差）|

**Strongly Supported（標點／助詞／同文案換聲優變體）10 槽：**

| 槽 | 套 | 依據 |
|---:|---|---|
| 02 | Voice A | 援護してくださいっ！↔援護してください（句尾差）|
| 04 | Voice Shuffle（中の人＝サイラス／桑原敬一）| 全 27 句＝標準同文案；Wiki 自註「再録」——**換聲優不換稿**，文本同文正是預測現象 |
| 05 | Voice Cafe | ご主人様、援護をお願いします（句讀差）|
| 06 | Voice Action | 援護・・・ヨロシクゥ！↔援護、よろしくぅ |
| 07 | Voice School | みんなの援護が欲しい↔ほしい |
| 08 | Voice A2 | よろしくお願いします！↔宜しくお願いします |
| 16 | Voice Police | 攻撃を受けています、援護を・・援護を（標點順序差）|
| 17 | Voice Idol | た･･弾が飛んできた～援護をぉぉぉ↔…！援護を |
| 46 | Voice Shuffle2（中の人＝ガイ／檜山修之）| 同 04 之同文案換聲優（Wiki 表格＝標準全文，自證）|
| 78 | Voice Shuffle3（中の人＝ドッドン／岡崎雅紘）| 同 04（同自證）|

**Strongly Supported 續（本輪新增 10 槽——Wiki 表空／單句／指引文限界）：**

| 槽 | 套 | 依據 |
|---:|---|---|
| 47 | Voice メカニカルウェア | 装甲が持たない、援護を！——Wiki 該套表空白，台詞語彙獨特 |
| 65 | Voice 応援団（16歳）| 押忍！喉がカラカラっす！／団員全員で突撃！＋Wiki 導文「応援団員」|
| 66 | Voice Oshigoto（20歳）| プレゼン資料がほしい・・・＋導文「社会人一年目」|
| 67 | Voice Santa（16歳）| トナカイがやられた！＋導文「戦場好きのサンタ」|
| 69 | Voice 男装＆女装（16歳）| 援護おねがいね♪／みんなで一緒に行こうよー——**女裝語氣**，剩餘女性語氣套唯一候位 |
| 70 | Voice バレンタイン（16歳）| チョコをくれないかな？＋導文「チョコレートが欲しすぎる」|
| 71 | Voice ひなまつり（19歳）| お内裏である！＋導文「おだいりさま人形」——詞彙互指 |
| 74 | Voice スポーツ（17歳）| みんな！来てくれ！／全員速攻！——直向元氣系；Wiki 表空白 |
| 79 | Voice コンバット | Wiki 唯一非空行 Z9「占領するぞ、突撃！」↔XML「占領するぞ！突撃！」；且與 76/77 的占領情報列結構相鄰 |
| 80 | Voice 執事・メイド（20歳）| 忙しすぎる～！援護お願いします！／なるほど、みんなで行くんですね？——新米勤務丁寧系；Wiki 表空白 |

#### コホート構造（本輪大定案——15 塊跨角色同文案矩陣，超出台詞對位的新結論）

對每一槽檢驗「哪些角色塊與 hayate 台詞全同」，出現穩定的共稿群組，
本身就是陣容演化的化石記錄（Fact：群成員逐槽檢驗；解讀為 Strongly Supported）：

- **男性陣 6 人組**＝{hayate, Cyrus, Doddon, Guy, van, hood}——恰好是全部
  6 名男性 PG。槽 **33、34、35、44、48、58、60、61、68、73、75、81**：
  男 6 共稿、9 女各自別稿（性別分裂系）；其套名**不出現在ハヤテ Wiki 頁**
  （Unknown；需讀サイラス等其他角色頁對名）；hayate 面判讀：
  33＝武士系（引導を渡してやろう）、34＝軍系（作戦を開始する）、
  35＝ライダー系、48＝捜査軍系、58＝愛色ナルシスト系、60＝粗暴系、
  61＝正義漢系、68＝現代軍系、73＝群れ討伐系、75＝囲城系、81＝ツッコミ系。
  （女 9 塊之別稿存在，例 tina V33＝戦いを始めます。）
- **14/15 共稿群（ルーシー除く全員）**：槽 **36–40、49–54**（共 11 槽）
  及 **26 イカ娘** —— devilgirl 處皆 ダミー或別案。與 sounds80 獨收
  devilgirl 呼應：**ルーシー為最後追加角色，其塊在早期系列槽全部缺稿**——
  年代化石（§5e-2 方法論的音源版先例）。
- **女性 8 人專用槽**（hayate 與男 6＋ルーシー皆 ダミー）：**62、63、64、82**——
  僅 tina/milly/Tericia/alulu/spy_11/robotgirl_12/tsunderegirl/magicgirl
  有實稿（例 milly V62＝わっ！援護お願いできますー？）。
  先前「hayate ダミー槽」記述由此更正：**62–64、82 非空殼，是女陣專用**。
- **9/15 群**＝hayate＋女 8（ルーシー除）：**84/85 方言包**——與 2015 CASH
  公告「※ハヤテを除く男性キャラクターにはございません」字面吻合
  （ハヤテ＋女性陣為對象；ルーシー槽為 ダミー，未細查其替代走向）。
- **11/15 群**：**25 Army（英語）** 11 塊共稿（異稿＝アルル/レム/エリス/
  ルーシー 各別）。
- **小群實稿槽（hayate 空殼但屬正常設計）——各 27 格滿寫：**
  83 僅 {milly, hood, tsunderegirl, magicgirl}、
  86 僅 {tina, milly, Guy, Tericia}、88 僅 {milly}、
  89 僅 {tina, milly}、90 僅 {devilgirl}、92 僅 {milly}——
  均為非直觀子集，語境 Unknown（buf 的槽位本身並非佔位）。
- **部分實裝槽（男性 6 共稿）**：**76、77** —— command/tactics 列（0..17）
  全 ダミー、唯 **information 列（18..26）實稿**。76＝大人正經報告
  （含「占領開始！」）、77＝幼兒口吻報告（たおしたー 等）——與 79 コンバット
  構成**占領戦／特殊模式專用情報 voice 帶**（Probable）。
- **唯一全空槽**：41（15 塊 × 27 格全 ダミー）；**87 號 XML 內跳缺**

**判讀量統計（hayate 92 槽最終構成）：** 已定名 54＝Confirmed 34 ＋
Strongly Supported 20（§5c-2b 的初期 11 套販售包恰佔連槽 01–11、
S2/S3 緊接 12–13——**槽號≈投入時序**，與 §5e-2 日期考古互證）＋
コホート結構已判讀 26（男 6 系 12＋14 群 11＋部分實裝 2＋孤件 1：
**32＝怪盗/シーフ系台詞槽**「アジトがやばい…シーフの腕を見せてやろう！」
——Wiki 53 套名冊無對應，疑未公開・取消套，對應 §5e-2 族②同型現象）＋
hayate 面純佔位 12（41,62,63,64,82,83,86,88–92；62–64/82 為女 8 專用、
83/86/88–90/92 各有小群實稿——均屬正常設計，唯 41 為真全空殼）
＋跳缺 1（87）＝92 槽全對號。
殘留議程：コホート系列套名（男 6 系、女 8 系、14 群系）待讀

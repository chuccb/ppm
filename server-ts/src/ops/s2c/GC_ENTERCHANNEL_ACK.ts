 ...
 * Type 3 enters the fully recovered `sub_875680` continuation (including
 * fixed-width raw4 fields and bounded/unbounded count loops); it is emitted
 * only when an explicit raw tail is supplied. (docs/PACKETS.md §3.15d5;
...
function writeType3Tail(p: Packet, tail: Type3Tail): void {
  ...
  if (tail.name.length > 0x7fff) throw new RangeError("196 type3.name is too long");
  ...
  requireS32("type3.listCount", tail.listCount, 0);
  if (tail.listValues.length !== tail.listCount) {
  ...
  tail.stageRecords.forEach((record, index) => {
    ...
    if (record.hasName0 === 0 && record.name0 !== undefined) {
      throw new RangeError(`196 type3.stageRecords[${index}].name0 requires hasName0`);
    }
    ...
    if (record.hasName1 === 0 && record.name1 !== undefined) {
      throw new RangeError(`196 type3.stageRecords[${index}].name1 requires hasName1`);
    }
  ...
  p.s32(tail.header0).s32(tail.header1).str(tail.name)
